...
<details><summary>Open for full command</summary>
...
* To test your server you can:
	- Run it and open `localhost:3000/users` in the browser
...
In the `dating_app` top-level directory, **above** your rails app `create-react-app dating_app_client`
...
Remove the Create React BoilerPlate from `App.js` and add in the `Hello` component that you built:
...
Alternatively, set the proxy in `package.json` and use relative routes like `/daters`.
...
**RESTART your Rails app: it may NOT work**

If CORS is broken, that's because we didn't uncomment the `rack-cors` gem.
...
* Refer to the class notes from today for how to make a Flexbox/Grid layout with header, footer, columns, main content, etc. Or, use this [tutorial](https://philipwalton.github.io/solved-by-flexbox/demos/holy-grail/).
...
The form should `submit` to a function that will process the form data. So, you will want your inputs each to bind to a `state` property in the `model` of that form data.
