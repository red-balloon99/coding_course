...
1. The instructor version
    - accessible to you on github. The github version is known as the 'remote' version or 'on/in the cloud' or 'on the internet'
      - you can look at the repo in the browser by going to the url: `https://git.generalassemb.ly/sei-flex/dan-abramov`
      - you can get the contents onto your computer (local version) by `git pull upstream master` (we set this up yesterday)
        ...
        - `upstream` is just an alias (another name) for the url location of our repo. Computer programers love to work smarter and not harder. By renaming the url to something short, informative and memorable it is much easier to type `upstream` than having to type `https://git.generalassemb.ly/sei-flex/dan-abramov/tree/master/unit_1/w02d01/warm_up_exercise` every single time you want to get the changes
...
- If you have not yet changed you default text editor, [this link](https://stackoverflow.com/questions/2596805/how-do-i-make-git-use-the-editor-of-my-choice-for-commits) shows you how.

**Fix**

- `git pull origin master`
- If Vim opens, type `:wq`
- If a tab pops up in your text editor, close it
...
**Use extreme caution** `rm -rf` is a very dangerous command that can irreversibly delete anything on your computer, so make sure to specify the exact directory you want to delete. For the first couple weeks of class, confirm with the instructional staff that you are running this command correctly.

### One More Gotcha!

**GOTCHA:** Do not initialize a git repository inside of another git repository. And definitely do not make your home (`~`) or root (`/`) directory a git repository - this will slow down your computer, confuse git, and you won't be able to track or even open your files on Github.
...
5) Whom should you talk to before running `rm -rf` somewhere?
...
#### Activity: More about HTML Documents
...
<!--Do this as a partner exercise, then have someone in each pair Slack them-->
...
Note: I have a `boilerplate` folder already created within the `warm_up_exercise` directory with the files and folders that you will need. Feel free to reference this, but **DO NOT** copy.

Inside today's `warm_up_exercise` folder:
...
- touch `app.js`
- open `app.js` to edit the file in Atom `atom app.js`
- make a `console.log` in the JS file so that you can verify that it is hooked up. So, on line 1 of your `app.js` file, write the code `console.log("My app.js file is attached")`.
...
2) Write your HMTL code (see above and reference the videos). HINT: Atom has a built-in way to create an HTML boilerplate! On line 1 of your `index.html` file, type `html` and hit `enter`. Note: make sure that you do not put a space after `html`.
...
5) Add a link to your `style.css` (where does it go? In the head? in the body?)
