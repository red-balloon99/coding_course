...
Duration: "0:45"<br>
<!--REALLY?!?! this file is 450 lines long-->
...
<!--Recommend this to folks who want to improve their CSS workflow for HW -->

## Fluid Layouts and Floats Intro

In order to build web sites that look good on any browser, can be resized, in any orientation, CSS had to add new features.
...
In this lesson, we'll be looking at how to use fluid layouts and floats in a modern way, and save other layout techniques that are better suited for newer technologies (like `flexbox`) for later lessons.

## Fluid Layout Build

<!--Raise your hand if you've started dreaming in code?  Chances are everyone will raise their hand by the end of class-->
...
- Navigate to `w01d03/student_examples/fluid_floats` in your local copy of this repo
- Open `index.html`
...
Let's put Kono and Vashti's dreams next to each other. Both are divs that have a class of `half` (we'll also include a lot of the same rules to the dreams with a class of `third`)
...
<!--Check students are caught up with thumbs-ups -->

## Intro to Floats

The property of `float` allows an element to be floated out of its default alignment and other elements will float around it. An excellent use case for floats is to use them to get text to wrap around an image.
...
Let's take a step away from our project to look at a simple example of floats. You have these examples, but just watch me code them out.
...
<!--Think about float as kind of like a vacuum on the given side. Unfortunately, floating something takes it out of the standard document flow, which is what led to the overlapping elements at the beginning.  Elements that are left *in* the document flow will try to fit wherever they can...unless we turn those vacuums off, which is what finally put the Footer in the "right" place.-->
...
There isn't really an intuitive and clean way to solve this. Thankfully, other people have solved this and you can just use their solution. You can solve this problem by using what is typically referred to as a `clear fix` or `clearfix`
...
<!--TLDR: a clearfix turns off all vacuums after all floated elements have been rendered-->

## A Good Use Case for Floats

Given the above trickiness of floats, they are best avoided. Modern CSS has much more elegant and simple solutions (we'll learn about them in upcoming lessons). But these techniques are good to know because there is a lot of legacy code out there and you'll definitely see these things.
...
By default, the size of most elements are measured by their content only. Padding, borders, and margins are not considered in the calculations.
...
<!--When editing styles for big HWs and projects, highly recommend the universal border strategy, then comment it out when you're almost done-->
