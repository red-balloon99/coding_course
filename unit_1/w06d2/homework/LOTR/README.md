...
3. Note that each chapter has a button in your browser with a click handler attached in the html file, and an associated javascript function in `fellowship.js`.
...
NOTE: Each function depends on the previous ones, so make sure each function works before moving on to the next one!
