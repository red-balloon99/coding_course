...
1. Think about the larger solution **not** as a whole, but as a series of steps that you would write out for a petulant child to follow
...
```
Peeling an orange: First, do we have an orange?  If not, I'm going to take one
out of the fridge.  We now have the orange.  Then I'm going to see if it has
already been peeled.  If it is peeled, I'm going to eat it.  If it isn't
peeled, I'm going to remove a chunk of the rind.  At this point, I'm going to
see if it's peeled.  If it isn't I'm going to remove another chunk of the
rind.  I'll keep doing this until the orange is peeled.
...
<!--groups of 2 or 3 for this -->

### Activity (10 min) + review

Pseudo Code the process for driving a car out of a garage
...
1. Put a `console.log()` inside this file
...
Now any messages that you send using `console.log()` will appear here (next to the blue caret)
...
You can keep getting input until a certain value exists like so:
...
- This can be done with a start function that gets called at the beginning and restarting of a situation.
...
- Try to have your functions defined at the top of the javascript file, all together
