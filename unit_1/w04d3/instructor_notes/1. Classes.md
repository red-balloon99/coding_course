...
<!--Talk about procedural P vs OOP vs FP-->
<!--Breaks are really important this day--make sure you do more than usual-->
...
remember arrow functions cannot have a name
also, arrow functions do not use `this` the
expected way in this class -->
...
<!--Try drawing on the screen for this with Zoom -->
...
`constructor` is a special function. Try misspelling `constructor` (e.g. `constr`) and see if you still get the same results.
...
<!--Only instructor needs this in their editor-->
...
Our world would be very boring and weird if all of our people are exactly the same! We need a way to customize each object: Our constructor function can take params which we can use to alter the properties of the object instantiated. This allows us to customize each instance:
...
There are two ways to write this. Writing it in the constructor with an `=` is the newer way. Using `||` is the older way. In both cases, values that have default parameters should go at the end (why?).

<!--Ask someone to raise their hand here-->
...
<!--SEI1 10:40, plus questions to 10:42 -->
