...
Modal boxes are frequently utilized for login/register forms, advertisements, pop-outs of information, or just notifications to the user.
...
3. Add the elements that will go **inside** the modal `<div>`:
  ...
  - Add a heading tag for the modal's header with the text `Learn to Play Bridge`
  `<h1>Learn to Play Bridge</h1>`

  - Add three `p`aragraph tags with text inside the box.  Feel free to use lorem ipsum text or, for more authentic text, take some from here: http://www.acbl.org/learn_page/how-to-play-bridge/
...
1. In your `app.js` file, grab the three pieces we'll need: `About the Game` button, the modal, and the `close` button.  Write your code inside the jQuery document ready provided.
...
3. Adjust the close button location. Select just the footer and move the close button to the right:
  `text-align: right;`
...
1. Change the close `<a>` tag to a `<button>` tag so that we can use the same styling for both, and give _both_ `<button>` tags a **class** of "modal-buttons".
...
2. In the css file, add the following style:
