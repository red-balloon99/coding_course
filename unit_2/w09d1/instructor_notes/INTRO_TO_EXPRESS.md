...
<!--Hook: Remember when we were using web APIs like Marvel and Google Books?  Raise your hand if you'd like to make one of those yourself.  Either way (good/too bad), that's what we're doing today. -->

<!--Note: this is a BIG transition.  We're almost starting over, so you will feel very lost for the first couple weeks, then things will start looking familiar, and you'll realize why we're doing what we are.-->
...
<!--Ask Mark and Kevin if they've ever had to go into node_modules to troubleshoot something -->
...
<!--Key point -- this is LISTENING for a GET request, not sending one -->
...
<!--In other words, Zeb will not be using semicolons anymore-->
...
1. Install it with `npm install nodemon -g`
...
<!--     You might not be able to install node packages globally by default. You may have to run `sudo npm i nodemon -g` and enter your computer's username and password.  Or we could just chown some stuff.  Shrug. -->

When you start a new project and do `npm init` and go through the prompts, you can set this right away.

<!--SEI1 7:16 Need to cover this tomorrow-->
